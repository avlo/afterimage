...
class MultipleSubcsriberClassifiedListingEventMessageIT extends AbstractMultipleSubscriber {
  ...
  MultipleSubcsriberClassifiedListingEventMessageIT(
      ...
      @Value("${afterimage.test.req.hexCounterSeed}") String hexCounterSeed,
      @Value("${afterimage.test.req.hexNumberOfBytes}") Integer hexNumberOfBytes,
      @Value("${afterimage.test.req.instances}") Integer reqInstances) {
