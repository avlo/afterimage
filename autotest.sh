...
# duration (seconds) between afterimage service process start and nostr-java integration-test start
IT_WAIT=15
# IT_WAIT > ( time @ afterimage service process in "running state" ) - ( time @ afterimage service process start )  
...
SUPER_HOME=$GIT_HOME/afterimage
...
cd_afterimage() {
  banner "cd to afterimage:" "" "     [$SUPER_HOME]"
...
build_afterimage() {
  cd_afterimage
  banner "building afterimage..."
  invoke_builder || exit_with_code $FAILURE_EXIT_CODE
  banner "...completed afterimage build" 
}
...
terminate_afterimage() {
  cd_afterimage
  kill_pids "$SUPER_PID"
  banner "afterimage pid [$SUPER_PID] terminated, autotest complete"
  exit_with_code $1
}
...
run_afterimage_integration_tests() {
  banner "running afterimage integration tests..."
  cd_afterimage
  sleep .01
  invoke_integration_tests || terminate_afterimage $FAILURE_EXIT_CODE
  banner "...afterimage integration tests completed"
}
...
start_afterimage() {
  ...
  run_afterimage 
  sleep .01
  banner "starting afterimage service pid: [$SUPER_PID]" "& waiting [$IT_WAIT] seconds prior to starting nostr-java test"
  sleep $IT_WAIT
}
...
user_prompt() {
  while true; do
    read -p "afterimage integration-test script options:
  (m) -> maven
...
# unit & integration tested afterimage snapshot build
build_afterimage 
run_afterimage_integration_tests
